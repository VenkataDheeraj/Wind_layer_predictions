...
from hcipy.atmosphere import InfiniteAtmosphericLayer,MultiLayerAtmosphere
...
from hcipy import AngularSpectrumPropagator
...
def generate_gif(X, T, process='forward', base_folder='gif_images'):
    ...
    print(len(images))
    imageio.mimsave(f'gifs/{process}_process.gif', images)
...
def output_h5(run_l, layer_heights_l, winds_l, nlayers_l, location_l, month_l, coefficients_l, D, pixscale_wanted, λmin,
    ...
    detector = Detector(False, True, np.uint32, 1.0, 1.0, 2**12 - 1, 0.0, exptime)

    # Create the wavefront for the star image
    star_wavefront = Wavefront(Field(object_image.flatten(), pupil_grid), λmin)
    ...
    layers = []
    for cn2_value, height,wind in zip(cn2_profile, layer_heights, winds_l):
        layer = InfiniteAtmosphericLayer(pupil_grid, cn2_value, L0=L0, velocity=wind)  # Example velocity
        layers.append(layer)

    # Use MultiLayerAtmosphere to manage all layers together
    atmosphere = MultiLayerAtmosphere(layers)
    distances = layer_heights_l
    Angular_Spectrum_Propagator = [AngularSpectrumPropagator(pupil_grid, d) for d in distances]
    ...
    for i,t in enumerate(timestamps):
        ...
        # Combine the star wavefront with the initial image
        wavefront.electric_field *= star_wavefront.electric_field

        # Propagate through each atmospheric layer
        atmosphere.evolve_until(t)
        wavefront.electric_field *= np.exp(1j * atmosphere.phase_for(wavefront.wavelength))

        for propagator in Angular_Spectrum_Propagator:
    ...
    generate_gif(data, nframes,process='Angular_Spectrum_Propagator_with_Multilayer_gain=1')




    plt.imshow(data[1]-data[0], cmap='gray')
